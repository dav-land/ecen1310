...
void testSymmetric(void)
{
  ...
  A = newMatrix(3, 3); assert(A);
  err = setElement(A, 1, 1, 1.0); assert(!err);
  err = setElement(A, 1, 2, .35); assert(!err);
  err = setElement(A, 1, 3, -.1); assert(!err);
  err = setElement(A, 2, 2, .5); assert(!err);
  err = setElement(A, 2, 3, .3); assert(!err);
  err = setElement(A, 3, 2, .14); assert(!err);
  err = setElement(A, 3, 3, -.3); assert(!err);
  printf("\nMatrix A:\n");
  err = printMatrix(A); assert(!err);

  err = symmetric(A);  //should not be symmetrical in this case
  if(err == 1)
  ...
  B = newMatrix(3, 3); assert(B);
  err = setElement(B, 1, 1, 2.0); assert(!err);
  err = setElement(B, 2, 2, 3.0); assert(!err);
  err = setElement(B, 3, 3, 4.0); assert(!err);
  printf("\nMatrix B:\n");
  err = printMatrix(B); assert(!err);

  err = symmetric(B);  //should be symmetrical in this case
  if(err == 1)
...
void testGetColumnGetRow(void)
{
  ...
  Ac = getColumn(A, 1); assert(Ac);  //gets the first column of A
  printf("\nMatrix A column 1:\n");
  err = printMatrix(Ac); assert(!err);
  ...
  Br = getRow(B, 3); assert(Br);  //gets the thrid row of B
  printf("\nMatrix B Row 3:\n");
  err= printMatrix(Br); assert(!err);
...
void testScalarProduct(void)
{
  ...
  err = scalarProduct(2.0, A);  //every value in A should be doubled
  if(err == 0)
    {
      printf("\nMatrix A * 2:\n");
      err = printMatrix(A); assert(!err);
  ...
  B = newMatrix(3, 3); assert(B);
  err = setElement(B, 1, 1, 1.0); assert(!err);
  err = setElement(B, 2, 2, 1.0); assert(!err);
  err = setElement(B, 3, 3, 1.0); assert(!err);
  printf("\nMatrix B:\n");
  err = printMatrix(B); assert(!err);

  err = scalarProduct(0.0, B);  //every value in B should be 0
  if(err == 0)
    {
      printf("\nMatrix B * 0:\n");
      err = printMatrix(B); assert(!err);
...
matrix * getColumn(matrix const * mtx, int col){
  ...
  matrix * may = newMatrix(mtx->rows, 1);
  if(!may) return NULL;
  for(int i = 1; i <= mtx -> rows; i++){
    double val = getE(mtx, i, col);
    setE(may, i, 1, val);
  }
  return may;
}
...
matrix * getRow(matrix const * mtx, int row){
  ...
  matrix * may = newMatrix(1, mtx->cols);
  if(!may) return NULL;
  for(int i = 1; i <= mtx -> cols; i++){
    double val = getE(mtx, row, i);
    setE(may, 1, i, val);
  }
  return may;
}
